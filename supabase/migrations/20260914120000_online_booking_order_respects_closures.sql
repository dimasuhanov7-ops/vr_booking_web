-- booking_create_order уважает паузу приёма и закрытые окна.
--
-- Занятость из booking_busy_intervals — подсказка для интерфейса; запрет обязан
-- стоять на записи, иначе закрытое окно бронируется прямым запросом к RPC.
-- Продолжение миграции 20260913120000 (booking_availability).
--
-- Новые коды ошибок: INTAKE_CLOSED, SLOT_CLOSED.
...
begin
  ...
  -- Пауза приёма не касается сотрудника: он вносит брони по телефону и тогда,
  -- когда онлайн-запись закрыта.
  ...
  loop
    ...
    -- Закрытый зал или окно. Сотруднику тоже нельзя: он сам это и закрыл,
    -- а «передумал» — снимает закрытие в админке.
    if exists (
      select 1
...
comment on function public.booking_create_order is
  'Единственный способ создать бронь. Отрезки (p_segments), лимиты публичного '
  'контура (3/час, 5 активных на номер), пауза приёма и закрытые окна.';
