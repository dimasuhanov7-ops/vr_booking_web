...
| Ступени цены | «До 6 шлемов одна цена, от 7 — другая». Цена ступени применяется **ко всем шлемам сразу** (8 × 700, а не 6 × 800 + 2 × 700) — решение заказчика 2026-09-16. Хранится в `booking_prices.min_qty`, считается и на сервере (`booking_station_price(…, p_qty)`), и в виджете. Админка редактирует одну ступень для шлемов (порог + две цены). |
