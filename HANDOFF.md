...
### ⚠️ Три миграции лежат в репозитории, но НЕ применены к проду

Код с ними уже работает, а без них — не ломается (деградирует мягко):

| Файл | Что даёт | Без неё |
|---|---|---|
| `20260924120000_online_booking_staff_read_all` | сотрудник видит имена коллег (`booking_staff`) | в «Журнале» вместо имени — «сотрудник 1a2b3c4d» |
| `20260924120100_online_booking_reschedule` | RPC `booking_reschedule_order`: перенос брони и смена состава | «Сохранить» с новым временем → «Перенос брони ещё не включён на сервере» |
| `20260924120200_online_booking_discounts_admin` | политика сотрудника и журнал на `booking_discounts`, код уникален без учёта регистра, процент ≤ 100 | список промокодов пуст, «Добавить» → «Промокоды ещё не включены на сервере» |

Применять через MCP `apply_migration` по одной, затем переименовать файлы под
присвоенные версии (см. выше). **`booking-mirror` в репозитории новее
задеплоенной v9** — учитывает скидку промокода в сумме и пишет код в Telegram
и таблицу; нужен редеплой (`deploy_edge_function`), без него сумма в таблице —
без скидки.
...
- **промокоды** — `booking_discounts` (нужна миграция `discounts_admin`).
