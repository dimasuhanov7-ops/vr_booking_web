...
Колонки в порядке `row()` в функции: дата, время, клуб, состав (+ пакет и
промокод), имя, телефон, людей, статус, источник, стоимость, предоплата,
комментарий, создана, синхронизирована, ID заказа.
