...
### Почему кнопка «Забронировать» не «прилипает» к низу экрана

Виджет тянется по своей высоте, а прокручивает страницу **родитель** — про этот
скролл виджет ничего не знает, поэтому липкий нижний бар внутри iframe
невозможен в принципе. Это осознанное решение, а не недоделка.

Если на длинной странице сумма и кнопка уезжают за экран, липкость делается
на стороне сайта — обёрткой вокруг iframe:

```html
<div style="position:sticky;bottom:0">
  <iframe id="vr-booking" ...></iframe>
</div>
```

Либо своей кнопкой, которую сайт показывает по событию `vr-booking:step`
(шаг 4 — контакты заполнены) и по клику скроллит к нижней части фрейма.
